...
) => Promisable<undefined | PrepareRequestFunctionResult>
...
type MessageSocketEvents = {
  connectionClosed: (data: { connectionId: number; stats: ConnectionStats }) => void
  requestFailed: (data: { error: NodeJS.ErrnoException; request: http.IncomingMessage }) => void
}
 ...
 */
export class Server extends TypedEventEmitter<MessageSocketEvents> {
  port: number
  ...
  getHandlerOpts(request: http.IncomingMessage): HandlerOpts {
    ...
    this.log(
      (request.socket as Socket).proxyChainId,
   ...
   * @param handlerOpts
   */
  async callPrepareRequestFunction(
    request: http.IncomingMessage,
  ...
  sendSocketResponse(socket: Socket, statusCode = 500, caseSensitiveHeaders = {}, message = ''): void {
    try {
      const headers = Object.fromEntries(
        Object.entries(caseSensitiveHeaders).map(([name, value]) => [name.toLowerCase(), value]),
      )
      ...
      this.log(
        socket.proxyChainId,
        `Unhandled error in sendResponse(), will be ignored: ${(err as Error).stack || err}`,
      )
    }
  }
