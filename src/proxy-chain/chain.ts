 ...
 * Client <- Apify <- Upstream <- Web
 */
export const chain = ({ request, sourceSocket, head, handlerOpts, server, isPlain }: ChainOpts): void => {
  ...
  client.on('connect', (response, targetSocket, clientHead) => {
    ...
    if (response.statusCode !== 200) {
      ...
      if (isPlain) {
        ...
        const status =
          statusCode === 401 || statusCode === 407 ? badGatewayStatusCodes.AUTH_FAILED : badGatewayStatusCodes.NON_200
