...
const socksToUrl = (proxy: SocksProxy): string => {
  const { host, port, userId, password } = proxy
  return `socks${proxy.type}://${userId ? `${userId}:${password}@` : ''}${host}:${port}`
}
...
export const forwardSocks = async (
  ...
  new Promise(async (resolve, reject) => {
    ...
    const options: Options = {
      ...
      agent: new SocksProxyAgent(socksToUrl(proxy)),
    }
    ...
    const client = http.request(request.url!, options as unknown as http.ClientRequestArgs, async (clientResponse) => {
      try {
        ...
        response.writeHead(statusCode, clientResponse.statusMessage, validHeadersOnly(clientResponse.rawHeaders))
